...
#ifdef CONFIG_IPV6_TUNNEL_DSLITE
/* Softwire (DS-Lite) */
/* Functions which handle proc filesystem */
static void  procfs_exit(void);
static int   procfs_init(void);
static int   show_smtu(char *buffer, char **start, off_t offset, int length, int *eof, void *data);
static int   show_toggle(char *buffer, char **start, off_t offset, int length, int *eof, void *data);
...
/* Disabled this bcos the concept has changed a bit of PMTU	*/			  
...
#ifndef CONFIG_IPV6_TUNNEL_DSLITE
	__u32 mtu;
	__u16 len;
#endif
...
#ifndef CONFIG_IPV6_TUNNEL_DSLITE
	/* NOT Softwire (DS-Lite):  Comment this line for DS-Lite to allow ipv6 fragmentation
...
#ifndef CONFIG_IPV6_TUNNEL_DSLITE
	/* NOT Softwire (DS-Lite): Comment out for DS-Lite for ipv4 fragmentation stop*/
	if (skb_dst(skb))
...
ip6_tnl_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	case SIOCDELTUNNEL:
		...
		/* Softwire (DS-Lite): Here we can remove the pointer to tunnel and proc interface if required*/
		break;
	default:
		err = -EINVAL;
	}
...
#ifdef CONFIG_IPV6_TUNNEL_DSLITE
/* Softwire (DS-Lite) */
static int show_devicename(char *buffer, char **start, off_t offset, int length, int *eof, void *data)
{
        int size;
...
static int show_l2tpsmtu(char *buffer, char **start, off_t offset, int length, int *eof, void *data)
{
        int size;
...
static int show_ipv6smtu(char *buffer, char **start, off_t offset, int length, int *eof, void *data)
{
        int size;
...
static int show_overrideipv6smtu(char *buffer, char **start, off_t offset, int length, int *eof, void *data)
{
        int size;
...
static int show_overridesmtu(char *buffer, char **start, off_t offset, int length, int *eof, void *data)
{
        int size;
...
static int show_l2tpoverridesmtu(char *buffer, char **start, off_t offset, int length, int *eof, void *data)
{
        int size;
