 ...
 */
static void nattype_nte_debug_print(const struct ipt_nattype *nte, const char *s)
{
#if defined(NATTYPE_DEBUG)
 ...
 */
static bool nattype_packet_in_match(const struct ipt_nattype *nte, struct sk_buff *skb, const struct ipt_nattype_info *info)
{
	...
	if (nte->proto != iph->protocol) {
		DEBUGP("nattype_packet_in_match: protocol failed: nte proto : %d, packet proto: %d\n",
	...
	if (info->type == TYPE_ADDRESS_RESTRICTED) {
		if (nte->dest_addr != iph->saddr) {
			DEBUGP("nattype_packet_in_match: dest/src check failed: dest_addr: %pI4, src dest: %pI4\n",
	...
	/*
	 * In either EI or AR mode, the ingress packet's src port can be anything.
	 */
	nattype_nte_debug_print(nte, "INGRESS MATCH");
	DEBUGP("INGRESS MATCH: PKT: proto[%d], src[%pI4:<x>], dest[%pI4:%d]\n",
		iph->protocol, &iph->saddr, &iph->daddr, ntohs(dst_port));
	return true;
}
...
static bool nattype_compare(struct ipt_nattype *n1, struct ipt_nattype *n2)
{
	...
	/*
	 * LAN Source compare.  
	 * Since we always keep min/max values the same, just compare the min values.
	 */
	...
	if (n1->range.min.all != n2->range.min.all) {
		DEBUGP("nattype_compare: r.min mismatch: %d:%d\n", ntohs(n1->range.min.all), ntohs(n2->range.min.all));
		return false;
	}
...
static unsigned int nattype_forward(struct sk_buff *skb, const struct xt_action_param *par)
{
	 ...
	 */
	nte = (struct ipt_nattype *)kzalloc(sizeof(struct ipt_nattype), GFP_ATOMIC | __GFP_NOWARN);
	if (!nte) {
	...
	/*
	 * TOOD: Would it be better to get this information from the conntrack instead of the headers.
	 */
	if (iph->protocol == IPPROTO_TCP) {
	...
	nte->range.flags = (IP_NAT_RANGE_MAP_IPS | IP_NAT_RANGE_PROTO_SPECIFIED);
