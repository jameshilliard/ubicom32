/*
 * net/ipv4/netfilter/ipt_PORTTRIGGER.c
...
/*
 * porttrigger_refresh_timer()
 *	Refresh the timer for this object.
 */
static bool porttrigger_refresh_timer(struct ipt_porttrigger *pte, unsigned long extra_jiffies)
{
	BUG_ON(!spin_is_locked(&porttrigger_lock));
 ...
 */
static void porttrigger_timer_timeout(unsigned long in_trigger)
{
	struct ipt_porttrigger *pte = (void *)in_trigger;
	...
	const unsigned int src_ip)
{
	DEBUGP("src_ip = %pI4, pte->src_ip = %pI4 proto = %d, dport = %d in match\n", 
		&src_ip, &pte->src_ip, proto, dport);

	/*
	 * Check LAND attack. If the ingress packet's src ip is equal to the pte's src ip (which is the
	 * LAN PC's ip), ignore the packet. 
	 */
...
static unsigned int porttrigger_nat(struct sk_buff *skb, const struct xt_action_param *par)
{
	...
	DEBUGP("porttrigger_nat: src_addr: %pI4 dst_port = %d protocol = %d\n", 
	...
	list_for_each_entry(pte, &trigger_list, list) {
		...
		newrange = ((struct nf_nat_range)
			{ IP_NAT_RANGE_MAP_IPS, pte->src_ip, pte->src_ip, 
			{0}, {0} });
		spin_unlock_bh(&porttrigger_lock);
...
static unsigned int porttrigger_forward(struct sk_buff *skb, const struct xt_action_param *par)
{
	...
	ct = nf_ct_get(skb, &ctinfo);
	if (!ct) {
		DEBUGP("porttrigger_forward: conntrack not found\n");
		return XT_CONTINUE;
	}
	...
	dst_port = ct->tuplehash[dir].tuple.dst.u.all;
	protocol = ct->tuplehash[dir].tuple.dst.protonum;
	DEBUGP("porttrigger_forward: src_addr: %pI4 dst_port = %d protocol = %d\n", 
		&src_addr, dst_port, protocol);
	...
	if (info->mode == MODE_FORWARD_IN) {
		...
		list_for_each_entry(pte, &trigger_list, list) {
			 ...
			 * found the entry.
			 */
			if (!porttrigger_refresh_timer(pte, info->timer * HZ)) {
				break;
			}

			/*
			 * The entry is found and refreshed, but the
			 * entry values can be changed by another thread in the MODE_FORWARD_OUT case,
	 ...
	 */
	if (!porttrigger_packet_out_match(info->trigger_proto, protocol, &info->trigger_ports, dst_port)) {
		return XT_CONTINUE;
	}
	 ...
	 */
	pte = (struct ipt_porttrigger *)kzalloc(sizeof(struct ipt_porttrigger), GFP_ATOMIC | __GFP_NOWARN);
	if (!pte) {
	...
	list_for_each_entry(pte2, &trigger_list, list) {
		if (!porttrigger_packet_out_match(pte2->trigger_proto, protocol, &pte2->trigger_ports, dst_port)) {
			continue;
		}
		 ...
		 */
		if (!porttrigger_refresh_timer(pte2, info->timer * HZ)) {
			break;
		}
		
		/*
		 * We have an entry in the list with this dst_port (trigger port) and protocol,
		 * but the src ip may be different than this new one, so refresh the src_ip of the found pte.
		 */
		pte2->src_ip = ntohl(src_addr);

		/*
		 * It is not good to use debug message inside the lock, but we are
		 * changing the value, and we have to print it here by paying the price under debugs on.
		 */
	 ...
	 */
	init_timer(&pte->timeout);
	pte->timeout.data = (unsigned long)pte;
	pte->timeout.function = porttrigger_timer_timeout;
...
static unsigned int porttrigger_target(struct sk_buff *skb, const struct xt_action_param *par)
{
	...
	ct = nf_ct_get(skb, &ctinfo);
	if (!ct) {
		DEBUGP("porttrigger_target: conntrack not found\n");
		return XT_CONTINUE;
	}
	...
	DEBUGP("porttrigger_target: src_addr = %pI4 dst_addr = %pI4 protocol = %d\n", 
	...
	DEBUGP("porttrigger_target: mode = %s\n", modes[info->mode]);
...
static int porttrigger_check(const struct xt_tgchk_param *par)
{
	...
	if (info->forward_proto != IPPROTO_TCP && info->forward_proto != IPPROTO_UDP && 
		...
		DEBUGP("porttrigger_check: bad trigger proto '%d'.\n", info->forward_proto);
		return -EINVAL;
	}
	...
	if ((info->forward_ports.pflags & (1 << IPT_MULTI_PORTS)) && 
		...
		DEBUGP("porttrigger_check: bad forward_ports list\n");
		return -EINVAL;
	}

	if ((info->trigger_ports.pflags & (1 << IPT_MULTI_PORTS)) && 
			(info->trigger_ports.ports[IPT_MULTI_PORTS - 1] != 65535)) {
		DEBUGP("porttrigger_check: bad trigger_ports list\n");
		return -EINVAL;
	}
