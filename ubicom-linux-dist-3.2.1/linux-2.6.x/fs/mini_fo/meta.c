...
int meta_build_lists(dentry_t *dentry)
{
	...
	if(dtohd2(dentry) && dtohd2(dentry)->d_inode) {
		mutex_lock_nested(&dtohd2(dentry)->d_inode->i_mutex, I_MUTEX_CHILD);
...
int meta_list_add_d_entry(dentry_t *dentry, const char *name, int len)
{
	...
	inode_info = itopd(dentry->d_inode);
		...
		kmalloc(sizeof(struct deleted_entry), GFP_NOFS);
        del_entry->name = (char*) kmalloc(len, GFP_NOFS);
			  ...
			  const char *new_name, int new_len)
{
	...
	inode_info = itopd(dentry->d_inode);
		...
		kmalloc(sizeof(struct renamed_entry), GFP_NOFS);
        ren_entry->old_name = (char*) kmalloc(old_len, GFP_NOFS);
        ren_entry->new_name = (char*) kmalloc(new_len, GFP_NOFS);
...
int meta_write_d_entry(dentry_t *dentry, const char *name, int len)
{
	...
	buf = (char *) kmalloc(len+3, GFP_NOFS);
	if (!buf) {
		printk(KERN_CRIT "mini_fo: meta_write_d_entry: \
                                  out of mem.\n");
		return -ENOMEM;
		       ...
		       const char *new_name, int new_len)
{
	...
	buf = (char *) kmalloc(buf_len, GFP_NOFS);
	if (!buf) {
...
int meta_sync_d_list(dentry_t *dentry, int app_flag)
{
	...
	/* here we go... */
		...
		/* size: len for name, 1 for \n and 2 for "D " */
		buf = (char *) kmalloc(del_entry->len+3, GFP_NOFS);
		if (!buf) {
			printk(KERN_CRIT "mini_fo: meta_sync_d_list: \
                                          out of mem.\n");
			return -ENOMEM;
...
char* __meta_check_r_entry(inode_t *inode, const char *name, int len)
{
	...
	inode_info = itopd(inode);
		...
		if(!strncmp(ren_entry->new_name, name, len)) {
			old_path = (char *)
				kmalloc(ren_entry->old_len+1, GFP_NOFS);
			strncpy(old_path,
				ren_entry->old_name,
				ren_entry->old_len);
