...
static int snd_ubi32_cs4384_setup_mclk(struct ubi32_cs4384_platform_data *pdata)
{
	...
	struct ubicom32_gpio_port *gpio = UBICOM32_GPIO_PORT(IO_PORT_PG5);
	unsigned int mask;
	int fn;
	...
	switch (pdata->mclk_src) {
		case UBI32_CS4384_MCLK_PWM_Q_G5_0:
			mask = (1 << 0);
			io = UBICOM32_IO_PORT(IO_PORT_RQ);
			fn = 2;
			break;
		...
		case UBI32_CS4384_MCLK_PWM_Q_G4_30:
			mask = (1 << 30);
			io = UBICOM32_IO_PORT(IO_PORT_RQ);
			gpio = UBICOM32_GPIO_PORT(IO_PORT_PG4);
			fn = 3;
			break;
		...
		case UBI32_CS4384_MCLK_PWM_R_G5_2:
			mask = (1 << 2);
			io = UBICOM32_IO_PORT(IO_PORT_RR);
			fn = 2;
			break;
		...
		case UBI32_CS4384_MCLK_PWM_S_G5_0:
			mask = (1 << 0);
			io = UBICOM32_IO_PORT(IO_PORT_RS);
			fn = 1;
			break;
		...
		case UBI32_CS4384_MCLK_PWM_T_G5_2:
			mask = (1 << 2);
			io = UBICOM32_IO_PORT(IO_PORT_RT);
			fn = 1;
			break;
...
#else
#error Processor not supported for this driver
#endif
...
static int snd_ubi32_cs4384_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	 ...
	 */
	err = snd_card_create(index, "Ubi32-CS4384", THIS_MODULE, 
			      sizeof(struct ubi32_snd_priv), &card);
	if (err < 0) {
		return err;
	}
