...
static int snd_ubi32_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	snd_printk(KERN_INFO "snd_ubi32_pcm_trigger cmd=%d\n", cmd);
#endif

	snd_pcm_group_for_each_entry(s, substream) {
		if (substream->pcm->card != s->pcm->card) {
			continue;
		}
					...
					struct snd_pcm_hw_params *hw_params)
{
	...
	if (ubi32_priv->pdata->set_rate) {
		...
		if (ret) {
			snd_printk(KERN_WARNING "Unable to set driver rate to %d, ret=%d\n", params_rate(hw_params), ret);
			return ret;
		}
	}
	...
	adr->bytes_per_sample = snd_pcm_format_width(format) / 8;
...
/*
 * snd_ubi32_pcm_dsp_enable_proc_write
 */
static ssize_t snd_ubi32_pcm_dsp_enable_proc_write(struct file *file, const char *buf, size_t count, loff_t *data)
{
	struct proc_dir_entry *pde = PDE(file->f_dentry->d_inode);
	struct ubi32_snd_priv *ubi32_priv = (struct ubi32_snd_priv *)pde->data;
	struct audio_dev_regs *adr = ubi32_priv->adr;
	...
	if (page == '1') {
		adr->processing_flags |= AUDIO_DEV_PROCESSING_FLAG_DSP;
	} else {
		adr->processing_flags &= ~AUDIO_DEV_PROCESSING_FLAG_DSP;
	}
...
static struct file_operations snd_ubi32_pcm_dsp_enable_proc_fops = {
	.write = (ssize_t (*) (struct file *, const char __user *, size_t, loff_t *))snd_ubi32_pcm_dsp_enable_proc_write,
};
...
{
	struct snd_pcm *pcm = ubi32_priv->pcm;
	if (ubi32_priv->proc_root) {
		remove_proc_entry("enable", ubi32_priv->proc_root);
	}
...
int __devinit snd_ubi32_pcm_probe(struct ubi32_snd_priv *ubi32_priv, struct platform_device *pdev)
{
	...
	if (adr->version != AUDIO_DEV_REGS_VERSION) {
		snd_printk(KERN_WARNING "Ubi32PCM: This audio_dev_reg is not compatible with this driver\n");
		return -ENODEV;
	}
	...
	if (adr->caps & AUDIONODE_CAP_BE) {
		if (adr->bytes_per_sample_mask & (1 << 2)) {
			snd_ubi32_pcm_hw.formats |= SNDRV_PCM_FMTBIT_S16_BE;
		}
		if (adr->bytes_per_sample_mask & (1 << 3)) {
			snd_ubi32_pcm_hw.formats |= SNDRV_PCM_FMTBIT_S24_3BE;
		}
		if (adr->bytes_per_sample_mask & (1 << 4)) {
			snd_ubi32_pcm_hw.formats |= SNDRV_PCM_FMTBIT_S32_BE;
		}
	}
	if (adr->caps & AUDIONODE_CAP_LE) {
		...
		if (adr->bytes_per_sample_mask & (1 << 3)) {
			snd_ubi32_pcm_hw.formats |= SNDRV_PCM_FMTBIT_S24_3LE;
		}
		if (adr->bytes_per_sample_mask & (1 << 4)) {
			snd_ubi32_pcm_hw.formats |= SNDRV_PCM_FMTBIT_S32_LE;
		}
	}
	if (adr->caps & AUDIONODE_CAP_DSP) {
		snd_printk(KERN_INFO "Ubi32PCM: DSP supported\n");

		/*
		 * Create proc node
		 */
		ubi32_priv->proc_root = proc_mkdir("dsp", NULL);
		if (!ubi32_priv->proc_root) {
			snd_printk(KERN_WARNING "Ubi32PCM: Failed to make proc node\n");
		} else {
			struct proc_dir_entry *pde = create_proc_entry("enable", S_IWUSR, ubi32_priv->proc_root);
			if (!pde) {
				printk(KERN_WARNING "Failed to create enable node\n");
			} else {
				pde->data = (void *)ubi32_priv;
				pde->proc_fops = &snd_ubi32_pcm_dsp_enable_proc_fops;
			}
		}

	}
	if (adr->bytes_per_sample_mask & (1 << 1)) {
		snd_ubi32_pcm_hw.formats |= SNDRV_PCM_FMTBIT_S8;
	}

	snd_printk(KERN_INFO "Ubi32PCM: sample_mask:%08x\n",
		   adr->bytes_per_sample_mask);

	snd_printk(KERN_INFO "Ubi32PCM: rates:%08x bits:%u min:%u max:%u count:%d fmts:%016llx (%s)\n",
		   snd_ubi32_pcm_hw.rates,
		   adr->bytes_per_sample * 8,
