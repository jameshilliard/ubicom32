				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	cur_vol_left = 255 - cs4350_priv->volume[0];
	cur_vol_right = 255 - cs4350_priv->volume[1];
...
#ifndef CONFIG_SND_UBI32_AUDIO_CS4350_AUTO_MUTE
	/*
	 * Register 0x04: Mute Control
	 *	Turn off auto-mute to work around DAC Bug
	 */
	send[0] = 0x04;
	ret = i2c_master_send(client, send, 1);
	if (ret != 1) {
		snd_printk(KERN_ERR "Failed to turn off auto-mute on CS4350\n");
		goto fail;
	}
...
static int snd_ubi32_cs4350_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	 ...
	 */
	err = snd_card_create(index, "Ubi32-CS4350", THIS_MODULE, 
