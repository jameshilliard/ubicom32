...
int ubi32_ocm_skbuf_min = 10, ubi32_ocm_skbuf_max, ubi32_ocm_skbuf, ubi32_ddr_skbuf;
		...
		int *eof, void *data)
{
	...
	len = sprintf(&buf[len], "--- OCM SKB usage (min %d - max %d)\n"
			"(SKB in OCM) %d - (SKB in DDR) %d\n",
			ubi32_ocm_skbuf_min, ubi32_ocm_skbuf_max,
			ubi32_ocm_skbuf,
			ubi32_ddr_skbuf);
                           ...
                           unsigned long count, void *data)
{
	...
	ubi32_ocm_skbuf_min = v;
	if (ubi32_ocm_skbuf_max < ubi32_ocm_skbuf_min)
		ubi32_ocm_skbuf_max = ubi32_ocm_skbuf_min;
