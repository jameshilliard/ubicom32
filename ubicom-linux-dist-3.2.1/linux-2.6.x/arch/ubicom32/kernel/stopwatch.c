/*
 * arch/ubicom32/kernel/stopwatch.c
...
/*
 * stopwatch_cycles_convert()
 *	Convert cycles to microseconds.
 */
static inline unsigned long long stopwatch_cycles_convert(unsigned long long cycles, unsigned int scale)
{
	unsigned long long micro = ((cycles * scale) / stopwatch_freq_us);
	return micro;
}
...
static int stopwatch_open(struct inode *inode, struct file *f)
{
	...
	if (ret < 0) {
		printk(KERN_WARNING "[%s]: unable to create sequence file: %s\n",
		       __func__, f->f_dentry->d_name.name);
		return ret;
	}
...
static int __init stopwatch_init(void)
{
	...
	stopwatch_freq_us = processor_frequency() / 1000000;
	spin_unlock(&stopwatch_init_lock);
	return 0;
}
...
void stopwatch_show(struct stopwatch_instance *si, struct seq_file *p, 
			const char *s, unsigned int scale)
{
	unsigned long flags;
	unsigned long long min, avg, max;

	spin_lock_irqsave(&stopwatch_lock, flags);
	min = stopwatch_cycles_convert(si->min, scale);
	avg = stopwatch_cycles_convert(si->avg, scale);
	max = stopwatch_cycles_convert(si->max, scale);
	si->min = 0;
	si->avg = 0;
	si->max = 0;
	spin_unlock_irqrestore(&stopwatch_lock, flags);
	seq_printf(p, "   \t%llu\t%llu\t%llu\t%s\n", min, avg, max, s);
}
EXPORT_SYMBOL(stopwatch_show);

/*
 * stopwatch_update()
 *	Update min, avg, max values in the stop watch
 */
void stopwatch_update(struct stopwatch_instance *si, unsigned int sample)
{
	unsigned long flags;

	spin_lock_irqsave(&stopwatch_lock, flags);
	if ((si->min == 0) || (si->min > sample)) {
		si->min = sample;
	}
	if (si->max < sample) {
		si->max = sample;
	...
	if (si->avg == 0) {
		...
		si->avg = ((si->avg * (STOPWATCH_WEIGHT - 1)) + sample) / STOPWATCH_WEIGHT;
	}
	spin_unlock_irqrestore(&stopwatch_lock, flags);
}
EXPORT_SYMBOL(stopwatch_update);
 ...
 */
unsigned int stopwatch_register(const char *name, int count, stopwatch_show_t show) 
{
	unsigned long flags;
	struct stopwatch *sw;
