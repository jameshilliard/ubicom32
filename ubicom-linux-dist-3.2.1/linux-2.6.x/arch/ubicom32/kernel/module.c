			      ...
			      struct module *mod)
{
	...
	if ((hdr->e_flags & EF_UBICOM32_VMASK) != (1 << (UBICOM32_ARCH_VERSION - 3))) {
		printk(KERN_WARNING "Module %s was not compiled for "
			"ubicom32v%d, elf_flags:%x,\n",
			mod->name, UBICOM32_ARCH_VERSION, hdr->e_flags);
		return -ENOEXEC;
	}
	...
	for (i = 0; i < sechdrs[relsec].sh_size / sizeof(*rel); i++) {
		...
		case R_UBICOM32_HI24:
		{
			...
			*location = insn;
#else
			/*
			 * 25 bit relocation that is part of the MOVEAI
			 * instruction. The 25 bits come from bits 7 - 31 of the
			 * relocation. Theses bits eventually get split into 2
			 * fields in the instruction encoding.
			 *
			 * - Bits 7 - 27 of the relocation are encoded into bits
			 * 0 - 20 of the instruction.
			 *
			 *  - Bits 28 - 31 of the relocation are encoded into
			 *  bit 25 - 27 of the instruction.
			 */
			uint32_t valid25 = (v >> 7) & 0x1ffffff;
			insn = *location;

			insn &= ~(0x1fffff | (0xf << 24));
			insn |= (valid25 & 0x1fffff);
			insn |= ((valid25 & 0x1e00000) << 3);
			*location = insn;
#endif
		}
		break;
