...
//	DEFINE(STAT_IRQ, offsetof(struct kernel_stat, irqs));
	...
	DEFINE(PT_NESTING_LEVEL, offsetof(struct pt_regs, nesting_level));
	DEFINE(PT_THREAD_TYPE, offsetof(struct pt_regs, thread_type));

	DEFINE(PT_ORIGINAL_D0, offsetof(struct pt_regs, original_dn_0));
	DEFINE(PT_PREVIOUS_PC, offsetof(struct pt_regs, previous_pc));

#if defined(IP8000)
	DEFINE(PT_INT_MASK2, offsetof(struct pt_regs, int_mask2));
#endif
#if (UBICOM32_ARCH_VERSION >= 5)
	DEFINE(PT_UCSR, offsetof(struct pt_regs, ucsr));
#endif
