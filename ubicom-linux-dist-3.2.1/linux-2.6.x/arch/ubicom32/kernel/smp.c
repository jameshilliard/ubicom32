...
#else
static unsigned int smp_debug_lvl = 1000;
...
/*
 * The IPI(s) are based on a software IRQ through the PIC.
 */
...
{
	arch_spin_lock(&smp_ipi_lock.rlock.raw_lock);
	if (smp_needs_ipi == 0) {
		ubicom32_clear_interrupt(smp_ipi_irq);
	}
	arch_spin_unlock(&smp_ipi_lock.rlock.raw_lock);
 ...
 */
static void smp_eoi_vector(unsigned int irq)
{
...
void smp_reset_ipi(unsigned long mask)
{
	...
	arch_spin_unlock(&smp_ipi_lock.rlock.raw_lock);
	smp_debug(100, KERN_INFO "smp: reset IPIs for: 0x%lx\n", mask);
}
...
unsigned long smp_get_affinity(unsigned int irq, int *all)
{
	 ...
	 * round.  When all CPU(s) have called smp_eoi_vector(),
...
static void __init smp_mainline_start(void *arg)
{
...
#if defined(CONFIG_MMU)
	/*
	 * We need to enable the kernel address early in case something
	 * tries to reference mapped space before a process context is used
	 * on this CPU.
	 */
	context_assign_kernel();

	/*
	 * Convert the thread to running virtual.
	 */
	cpu_virtual();
#endif
	...
	/*
	 * Make sure that interrupts are disabled for this thread.
	 */
...
int __cpuinit __cpu_up(unsigned int cpu)
{
	...
	/*
	 * Make sure that the tid is setup.
	 */
	BUG_ON(p->tid == 0);
	 ...
	 */
	idle = (struct task_struct *)virt_to_phys(fork_idle(cpu));
	if (IS_ERR(idle)) {
		panic("cpu[%d]: fork failed\n", cpu);
		return -ENOSYS;
	}

	ti = (struct thread_info *)virt_to_phys(task_thread_info(idle));
	ti->cpu = cpu;

	/*
	 * Setup the sw_ksp[] to point to this new task.
	 */
	sw_ksp[p->tid] = (unsigned int)virt_to_phys(idle->stack);
...
void __init smp_prepare_cpus(unsigned int max_cpus)
{
	 ...
	 */
	set_irq_chip_and_handler(smp_ipi_irq, &ubicom32_smp_chip, handle_percpu_irq);
...
void __init smp_setup_processor_id(void)
{
	...
	/*
	 * Take this early opportunity to set the affinity
	 * for all IRQs to all CPUs.
	 */
	for (i = 0; i < NR_IRQS; i++) {
		smp_irq_affinity[i] = 0xffffffff;
	}
}
