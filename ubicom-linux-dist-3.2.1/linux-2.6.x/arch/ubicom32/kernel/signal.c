			    ...
			    sigset_t *set, struct pt_regs *regs)
{
	...
	err |= __put_user( (  (current_thread_info()->exec_domain)
			   ...
			   : sig, &frame->sig);
	err |= copy_to_user (&frame->info, info, sizeof(siginfo_t));
	...
	if ((regs->frame_type == UBICOM32_FRAME_TYPE_TRAP) ||
	    ...
	    (regs->frame_type == UBICOM32_FRAME_TYPE_PAGE_FAULT)) {
	...
	/*
	 * Copy the signal_trampoline code into the created frame.
	 */
	err |= copy_to_user (frame->sig_tramp, signal_trampoline, 6*4);

	if (err)
		goto give_sigsegv;

	/*
	 * Flush and invalidate the signal trampoline that we have loaded into the signal frame.
	 */
	flush_icache_range((unsigned long) &frame->sig_tramp[0], (unsigned long) &frame->sig_tramp[7]);
	...
	regs->pc = (unsigned long) frame->sig_tramp;
	regs->an[5] = (unsigned long) frame->sig_tramp;
	regs->dn[0] = sig;
