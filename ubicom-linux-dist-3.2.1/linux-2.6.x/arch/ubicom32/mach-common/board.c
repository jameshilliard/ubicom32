...
/*
 * board_get_identity_string()
 *	Returns identity string of the board.
 */
const char *board_get_identity_string(void)
{
	if (!bn) {
		return "NULL";
	}

	return bn->identity_string;
}
...
void __init board_init(void)
{
	...
	if (bn->version != BOARDNODE_VERSION) {
		printk(KERN_WARNING "Board node is wrong version\n");
		bn = NULL;
		return;
	}
}
