 ...
 * operations or by the PIC to avoid disabling a thread performing an atomic
 * operation.
 *
...
/*
 * In an optimized build the following is not a real function.
 * The compiler should remove the function call as long as the user does not
 * pass in a size that __xchg and __cmpxchg are not prepared for.
 * If the user does pass in an unknown size, the user will get a link time
 * error. In non-optimized builds the function is real and calls BUG().
 *
...
#define __xchg_called_with_bad_pointer(...) BUG()
