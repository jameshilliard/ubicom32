...
#define MAX_UBICOM_ONCHIP_GPIO   (NUM_GPIO_PORT * 32)
...
#define UBICOM32_GPIO_GET_PIN(pin)				\
	(0 != (UBICOM32_PORT_NUM_TO_GPIO(gpio_bank(pin))->gpio_in	\
	 & gpio_bit(pin)))
...
#elif defined(IP7000)
	if ((gpio >= GPIO_RA_4) && (gpio <= GPIO_RA_6)) {
		return 32 + 12 + (gpio - GPIO_RA_4);
	}
#elif defined(IP8000)
	if ((gpio >= GPIO_INT_0) && (gpio <= GPIO_INT_3)) {
		return 32 + 23 + gpio;
	}
#endif
...
extern struct ubicom32_gpio_port *ubi_gpio_get_port(unsigned gpio);

extern int __init ubi_gpio_init(void);

#if defined(IP8000)
#define GPIO_INT_EDGE_NONE	0x00
#define GPIO_INT_EDGE_FALLING	0x01
#define GPIO_INT_EDGE_RISING	0x02
#define GPIO_INT_EDGE_ANY	0x03
extern int ubi_gpio_setup_int(unsigned int intr, unsigned int gpio, int mode);
#endif
