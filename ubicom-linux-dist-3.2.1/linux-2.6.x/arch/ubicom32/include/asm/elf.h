 ...
 */
#define EF_UBICOM32_VMASK	0x0000ffff
...
/*
 * We don't need anything here because we currently only support FDPIC.
 */
...
/*
 * This page size of the ELF is independent of the page size in the
 * kernel; however, it must be a multiple of the kernel page size.  This
 * approach means that you can build ELF files for 16K and have the
 * kernel support 4, 8, or 16K page size in the kernel.
 */
#define ELF_EXEC_PAGESHIFT	14
#define ELF_EXEC_PAGESIZE	(1 << ELF_EXEC_PAGESHIFT)

#ifdef __KERNEL__
#define ELF_FDPIC_CORE_EFLAGS	(EF_UBICOM32_FDPIC | ( 1 << (UBICOM32_ARCH_VERSION - 3)))
#endif

/* 
 * This is the location that an ET_DYN program is loaded if exec'ed.
 * Typical use of this is to invoke "./ld.so someprog" to test out a new
 * version of the loader.  We need to make sure that it is out of the
 * way of the program that it will "exec", and that there is sufficient
 * room for the brk.
 */
...
/*
 * ELF_CORE_COPY_TASK_REGS is needed to dump register state from multi
 * threaded user projects.
 */
struct task_struct;
 ...
 * specific libraries for optimization.  This is more specific in intent
 * than poking at uname or /proc/cpuinfo.
 */
