...
static inline thread_t thread_get_self(void)
{
	...
	/*
	 * In an SMP configuration, the thread value can change whenever
	 * schedule is called (or when we are preempted in the kernel).
	 * If you do not specify volatile, the compiler can cache the 
	 * ROSR value and you will wind up with the wrong thread number
	 * when returning from schedule or an interrupt.
	 *
	 * In the UP configuration, counter intuitively, the size of the
	 * code decreases when using volatile.  This is the result of 
	 * stack pressure introduced in the non-volatile case.
	 */
	asm volatile (
	"	lsr.4	%0, ROSR, #2	\n\t"
	"	and.4	%0, #0x1f, %0	\n\t"
...
/*
 * thread_enable_mask_nolock()
 *	Enable all threads in the mask.
 *
 * All writes to MT_EN must be protected by the MT_EN_LOCK bit
 * This routine can only be called while holding the MT_EN_LOCK.
 */
static inline void thread_enable_mask_nolock(unsigned int mask)
{
	/*
...
/*
 * thread_is_mmu_blocked()
 *	Is the specified tid blocked by the MMU?
 */
static inline int thread_is_mmu_blocked(thread_t tid)
{
	int thread_mask = (1 << tid);
	if (*(volatile int *)MMU_BUS_ST0 & thread_mask) {
		if (*(volatile int *)MMU_BUS_ST2 & thread_mask) {
			return 1;
		}
	}
	return 0;
 ...
 *
 * lsl instruction only uses the least-significant 5 bits from the register
 * which specifies the shift amount.  This means that we don't have to mask
 * the thread number field because it is exactly 5 bits.
 */
