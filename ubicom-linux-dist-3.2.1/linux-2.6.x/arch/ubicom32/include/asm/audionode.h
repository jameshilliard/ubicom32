...
#define AUDIONODE_CAP_DSP	(1 << 2)

#define AUDIONODE_VERSION	8
struct audio_node {
...
#define AUDIO_DEV_REGS_VERSION 6
#define AUDIO_DEV_PROCESSING_FLAG_DSP	(1 << 0)
struct audio_dev_regs {
	...
	u32_t bytes_per_sample_mask;			/* A bit set in a particular position means we support this bytes_per_sample configuration */
	volatile u32_t bytes_per_sample;		/* The number of bytes per sample (i.e. 24-bit audio would be 3) */
	...
	volatile u32_t processing_flags;		/* Bits which tell what processing is enabled */
