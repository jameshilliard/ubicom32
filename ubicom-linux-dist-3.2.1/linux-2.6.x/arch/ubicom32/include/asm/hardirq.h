 ...
 * on a single CPU.  For Ubicom32, we have 64/96 IRQ sources.
 */

#if (NR_IRQS <= 64)
#define HARDIRQ_BITS	6
#elif (NR_IRQS <= 128)
#define HARDIRQ_BITS	7
#else
#error "Ubicom priocessor w/too many interrupts"
#endif
...
/*
 * See arch/ubicom32/kernel/irq.c:do_IRQ()
 * Interrupts are disabled when calling irq_exit().
 */
