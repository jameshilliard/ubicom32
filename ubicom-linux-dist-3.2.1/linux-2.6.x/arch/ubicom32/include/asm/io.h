...
#define PCI_DEV_ADDR_BASE (0xBC000000UL)
#define PCI_DEV_ADDR_SIZE ((unsigned long)(1 << 25))
#else /* #if (UBICOM32_ARCH_VERSION > 4) */
#define PCI_DEV_ADDR_BASE (0x80000000UL)   /* taking higher 2GB space */
#define PCI_DEV_ADDR_SIZE ((unsigned long)(1 << 31))
#endif  /* UBICOM32_ARCH_VERSION > 4 */
#define PCI_DEV_ADDR_MASK (~(PCI_DEV_ADDR_SIZE - 1))
#define IS_PCI_ADDRESS(x) (((unsigned long)(x) & PCI_DEV_ADDR_MASK) == PCI_DEV_ADDR_BASE)
...
extern u32 ubi32_pci_inl(unsigned long addr);
extern u16 ubi32_pci_inw(unsigned long addr);
extern u8 ubi32_pci_inb(unsigned long addr);
extern void ubi32_pci_outl(u32 value, unsigned long addr);
extern void ubi32_pci_outw(u16 value, unsigned long addr);
extern void ubi32_pci_outb(u8 value, unsigned long addr);

extern void ubi32_pcie_dma_lock(void __iomem *addr);
extern void ubi32_pcie_dma_unlock(void __iomem *addr);
...
#if !defined(CONFIG_PCI) || !defined(UBICOM32_V5)
#define inb(addr)    readb((void __iomem *)(addr))
#define inw(addr)    readw((void __iomem *)(addr))
#define inl(addr)    readl((void __iomem *)(addr))
#define outb(x,addr) ((void) writeb(x,(void __iomem *)(addr)))
#define outw(x,addr) ((void) writew(x,(void __iomem *)(addr)))
#define outl(x,addr) ((void) writel(x,(void __iomem *)(addr)))
#else
/* The assumption is the only place we need support for IO address space is for PCI devices */
#define inb(addr)    ubi32_pci_inb(addr) 
#define inw(addr)    ubi32_pci_inw(addr)
#define inl(addr)    ubi32_pci_inl(addr)
#define outb(x,addr) ((void) ubi32_pci_outb(x,addr))
#define outw(x,addr) ((void) ubi32_pci_outw(x,addr))
#define outl(x,addr) ((void) ubi32_pci_outl(x,addr))
#endif
