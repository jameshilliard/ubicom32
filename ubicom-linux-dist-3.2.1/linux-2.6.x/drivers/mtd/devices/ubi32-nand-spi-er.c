...
static int ubi32_nand_spi_er_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	DEBUG(MTD_DEBUG_LEVEL3, "%s: erase addr:%llx len:%llx\n", chip->name, instr->addr, instr->len);
	...
	if (instr->addr & (chip->device->erase_size - 1)) {
		DEBUG(MTD_DEBUG_LEVEL1, "%s: erase address is not aligned %llx\n", chip->name, instr->addr);
		return -EINVAL;
	}

	if (instr->len & (chip->device->erase_size - 1)) {
		DEBUG(MTD_DEBUG_LEVEL1, "%s: erase len is not aligned %llx\n", chip->name, instr->len);
		return -EINVAL;
	}

	if (chip->select) {
		chip->select(chip->appdata);
	}
	...
	while (instr->len) {
		...
		uint8_t stat;
		DEBUG(MTD_DEBUG_LEVEL3, "%s: block erase row:%x block:%x addr:%llx rem:%llx\n", chip->name, row, block, instr->addr, instr->len);
 ...
 *	Chip should be selected prior to entry.
 */
...
static int __devinit ubi32_nand_spi_er_probe(struct platform_device *pdev)
{
	...
	const struct ubi32_nand_spi_er_device *device;
	struct ubicom32_io_port *io = (struct ubicom32_io_port *)FLASH_PORT;
	struct ubi32_nand_spi_er_platform_data *pdata = pdev->dev.platform_data;
#ifdef CONFIG_MTD_PARTITIONS
	...
	for (i = 0; i < ARRAY_SIZE(ubi32_nand_spi_er_devices); i++) {
		/*
		 * When we run at higher clock frequencies it seems that the
		 * device will swap the ID values.  This might happen if the
		 * setup/hold time on CS is violated.  However, tests so far
		 * have limited the problem to this one command.
		 */
		if ((uint16_t) device->id == (uint16_t) ((id >> 8) | (id << 8))) {
			break;
		}
	...
	if (i == ARRAY_SIZE(ubi32_nand_spi_er_devices)) {
		...
		io->ctl0 &= ~IO_XFL_CTL0_MCB_LOCK;
		printk("ubi32-nand-spi-er: failed to find id %x\n", id);
		return -ENODEV;
	}
	...
	if (pdata) {
		chip->select = pdata->select;
		chip->unselect = pdata->unselect;
		chip->appdata = pdata->appdata;
	}
	...
	if (pdata && pdata->name) {
		strncpy(chip->dev_name, pdata->name, sizeof(chip->dev_name));
	} else {
		strncpy(chip->dev_name, dev_name(&pdev->dev), sizeof(chip->dev_name));
	}
	chip->dev_name[sizeof(chip->dev_name) - 1] = 0;
	chip->mtd.name = chip->dev_name;
	...
	printk(KERN_INFO "%s: added device size: %llu KBytes %lu bad blocks %s\n", chip->mtd.name, DIV_ROUND_UP(chip->mtd.size, 1024), chip->nbb, read_only ? "[read only]" : "");

	/* 
	 * partitions should match sector boundaries
	 */
#ifdef CONFIG_MTD_PARTITIONS
	nr_parts = parse_mtd_partitions(&chip->mtd, part_probes, &parts, 0);
	...
	if (nr_parts > 0) {
		for (i = 0; i < nr_parts; i++) {
			DEBUG(MTD_DEBUG_LEVEL2, "%s: partitions[%d] = "
						...
						".size = 0x%.8llx (%lluKiB) }\n",
						chip->mtd.name, i, parts[i].name,
						parts[i].offset,
						parts[i].size,
						parts[i].size / 1024);
		}
		return add_mtd_partitions(&chip->mtd, parts, nr_parts);
	} else
#endif
	{
...
#ifdef CONFIG_MTD_PARTITIONS
	status = del_mtd_partitions(&chip->mtd);
#else
...
#ifdef UBI32_NAND_SPI_ER_AUTO_LOAD
...
static struct platform_driver ubi32_nand_spi_er_driver = {
	...
	.remove		= __devexit_p(ubi32_nand_spi_er_remove),
};
