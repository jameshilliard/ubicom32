...
#define UBICOM32_USE_NAPI
#define UBICOM32_POLL_BUDGET 64
...
#define ETHERNET_EXTRA	 (NET_IP_ALIGN + 2 * CACHE_LINE_SIZE + 4)	// Magic 4 for wireless TX alignment
...
#define DEFAULT_MAC_ADDRESS {0x00, 0x03, 0x64, 0xDE, 0xFA, 0xDD}
...
{
	u32 NbrBase;		/* base address of Ubicom NBR registers  */
	...
	struct vlan_group *vlgrp;	/* for VLAN acceleration */
#endif
	...
	struct ethtionode *ethdn;	/* Ubicom32 device tree node */
} synopGMACdevice;
...
/**********************************************************
 * Ubicom specific DMA map/unmap
 **********************************************************/
static inline void dma_unmap_single_desc(dma_addr_t desc)
{
	/*
	 * Only flush is needed
	 * (a pipe_flush delay to prevent immediately followed access).
	 */
	asm volatile (
	"	flush		(%[addr])	\n\t"
	"	pipe_flush	0		\n\t"
...
static inline void dma_map_single_desc(void *desc)
{
	/*
	 * The delay before sync is to ensure that no speculative sync is called.
	 * The extra access [after sync] is needed.
	 */
	asm volatile (
	"	flush		(%[addr])	\n\t"
	"	pipe_flush	0		\n\t"
...
static inline void dma_stall_gmac_trans(synopGMACdevice *gmacdev)
{
	/*
	 * Let's stop DMA by issuing DMA stall through Ubicom I/O NBR
	 */
	asm volatile (
	"	bset	"D(IO_CTL0)"(%0), "D(IO_CTL0)"(%0), #0	\n\t"
	"	pipe_flush	0				\n\t"
	"	btst	"D(IO_STATUS0)"(%0), #0			\n\t"
	"	jmpeq.f	.-4					\n\t"
		:
		: "a" (gmacdev->NbrBase)
		: "cc"
...
static inline void dma_resume_gmac_trans(synopGMACdevice *gmacdev)
{
	/*
	 * Let's resume DMA by clearing DMA stall through Ubicom I/O NBR
	 */
	asm volatile (
	"	bclr	"D(IO_CTL0)"(%0), "D(IO_CTL0)"(%0), #0	\n\t"
		:
		: "a" (gmacdev->NbrBase)
		: "cc"
	);
...
static inline DmaDesc *synopGMAC_get_tx_qptr(synopGMACdevice * gmacdev)
{
	...
	if(synopGMAC_is_desc_owned_by_dma(txdesc)) {
		dma_unmap_single_desc(virt_to_phys(txdesc));
		return NULL;
	}
...
static inline void synopGMAC_reset_tx_qptr(synopGMACdevice * gmacdev)
{
	...
	asm volatile ("sub.4 %0, %0, %1" : : "U4"(gmacdev->BusyTxDesc), "d"(1) : "cc", "memory");
}
 ...
 * @param[in] u32 indicating TX control flag - the first, last segment and interrupt state.
 * @param[in] u32 indicating descriptor DMA flag state.
 * \return returns present tx descriptor index on success. Negative value if error.
 */
static inline s32 synopGMAC_set_tx_qptr(synopGMACdevice * gmacdev, u32 Buffer1, u32 Length1, u32 Data1, u32 offload_needed, u32 tx_cntl, u32 set_dma)
{
	...
	#ifdef ENH_DESC
	txdesc->status |= tx_cntl; //ENH_DESC
	#else
	txdesc->length |= tx_cntl;
	#endif
	...
	#ifdef ENH_DESC
	txdesc->status |= set_dma;//ENH_DESC
	#else
	...
	/*
	 * Due to support of SG, the BusyTxDesc will be incremented by
	 * the number of fragments after all fragment DMAs are ready.
	 */
	//(gmacdev->BusyTxDesc)++; //busy tx descriptor is reduced by one as it will be handed over to Processor now
...
static inline s32 synopGMAC_set_rx_qptr(synopGMACdevice * gmacdev, u32 Buffer1, u32 Length1, u32 Data1)
{
	...
	asm volatile ("add.4 %0, %0, %1" : : "U4"(gmacdev->BusyRxDesc), "d"(1) : "cc", "memory");
...
static inline DmaDesc *synopGMAC_get_rx_qptr(synopGMACdevice * gmacdev)
{
	...
	if(synopGMAC_is_desc_owned_by_dma(rxdesc)) {
		dma_unmap_single_desc(virt_to_phys(rxdesc));
		return NULL;
	}
	BUG_ON(synopGMAC_is_desc_empty(rxdesc));
...
static inline void synopGMAC_reset_rx_qptr(synopGMACdevice * gmacdev)
{
	...
	asm volatile ("sub.4 %0, %0, %1" : : "U4"(gmacdev->BusyRxDesc), "d"(1) : "cc", "memory");
}
