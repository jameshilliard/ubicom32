...
extern int ubi32_ocm_skbuf_min, ubi32_ocm_skbuf_max, ubi32_ocm_skbuf, ubi32_ddr_skbuf;
static int ubi32_ocm_tx_skbuf;
...
#if (defined(CONFIG_ZONE_DMA) && defined(CONFIG_UBICOM32_OCM_FOR_SKB))
		if (!((int)skb->data & SDRAMSTART)) {
			ubi32_ocm_tx_skbuf++;
			if ((ubi32_ocm_tx_skbuf & 0x1f) == 0)
				if (ubi32_ocm_skbuf_max > ubi32_ocm_skbuf_min)
					ubi32_ocm_skbuf_max--;
		}
#endif
...
static int ubi32_eth_receive(struct net_device *dev, int quota)
{
	...
	if (replenish_cnt > 0) {
		...
		do {
			...
			ubi32_ocm_skbuf++;
			if ((ubi32_ocm_skbuf & 0x3f) == 0)
				if (ubi32_ocm_skbuf_max < RX_DMA_MAX_QUEUE_SIZE)
					ubi32_ocm_skbuf_max++;
 ...
 *	polling routine used by tasklet
 */
...
/*
 * ubi32_eth_link_state_poll()
 *	polling routine to check link state
 */
static inline void ubi32_eth_link_state_poll(struct net_device *dev)
{
	struct ubi32_eth_private *priv = netdev_priv(dev);

	if (likely(priv->regs->status & UBI32_ETH_VP_STATUS_LINK)) {
...
static int ubi32_eth_open(struct net_device *dev)
{
	...
	if (priv->regs->status & UBI32_ETH_VP_STATUS_LINK) {
		netif_carrier_on(dev);
	} else {
		netif_carrier_off(dev);
	}
...
static int ubi32_eth_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
	...
	case SIOCGMIIREG:
		if ((data->reg_num & 0x1F) == MII_BMCR) {
			...
			printk(KERN_INFO "%s: ioctl %d (unsupported MII reg %d)\n",
				 dev->name, cmd, data->reg_num);
			return -EIO;
		}
		break;

	default:
		printk(KERN_INFO "%s: ioctl %d (not yet)\n", dev->name, cmd);
		return -EOPNOTSUPP;
	}
...
static const struct net_device_ops ubi32_eth_netdev_ops = {
...
int ubi32_eth_init_module(void)
{
	...
	for (i = 0; i < UBI32_ETH_NUM_OF_DEVICES; i++) {
		...
		dev->netdev_ops         = &ubi32_eth_netdev_ops;
