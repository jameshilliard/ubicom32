...
#define DRIVER_NAME "ubicom32suart"

#ifdef IP8000
#define SERIAL_UBICOM_PIN_RXD	((1 << 0) << 8)
#define SERIAL_UBICOM_PIN_TXD	((1 << 6) << 8)
#else
#define SERIAL_UBICOM_PIN_RXD	(1 << 0)
#define SERIAL_UBICOM_PIN_TXD	(1 << 6)
#endif
#define SERIAL_UBICOM_CTL0	0x8a300000
...
#define IO_PORT_INT_SET(port) (IO_PORT(port)->int_set)
...
#ifdef IP8000
	struct ubicom32_gpio_port *gpio_port;
#endif
...
#ifdef IP8000
	/*
	 * WARNING: there's only one clock for the serdes, so if RO is using 250mhz, RP must also use 250mhz
	 * the driver does not check for this...
	 */
	*(unsigned int *)(GENERAL_CFG_BASE + GEN_CLK_IO_DISABLE) &=
		((unsigned int)port->membase == IO_PORT_RO) ? ~GEN_CLK_DISABLE_SERDES0 : ~GEN_CLK_DISABLE_SERDES1;
	switch (port->uartclk) {
	case 250000000:
		*(unsigned int *)(GENERAL_CFG_BASE + GEN_CLK_IO_SELECT) |= (1 << 29);
		*(unsigned int *)(GENERAL_CFG_BASE + GEN_CLK_IO_SELECT) |=
			((unsigned int)port->membase == IO_PORT_RO) ? (1 << 16) : (1 << 17);
		break;
	...
	default:
		printk(KERN_WARNING "Unsupported IO_CLK frequency %d\n", port->uartclk);
		return;
	}

	switch ((unsigned int)port->membase) {
	case IO_PORT_RP:
		gpio_port = UBICOM32_GPIO_PORT(IO_PORT_PG3);
		break;

	case IO_PORT_RO:
		gpio_port = UBICOM32_GPIO_PORT(IO_PORT_PG4);
		break;

	default:
		printk(KERN_WARNING "Unsupported port %p\n", port->membase);
		return;
	}

	gpio_port->gpio_ctl &= ~SERIAL_UBICOM_PIN_RXD;
	gpio_port->gpio_ctl |= SERIAL_UBICOM_PIN_TXD;
	gpio_port->fn_sel[2] |= SERIAL_UBICOM_PIN_RXD | SERIAL_UBICOM_PIN_TXD;
#else
	...
	/*
	 * Reset the RX FIFO, set the watermark to 1 character
	 */
	io_port->int_set = IO_PORTX_INT_FIFO_RX_RESET;
	io_port->fifo_watermark = IO_PORTX_FUNC_FIFO_RX_WM(1);
...
static void ubi32_serdes_handle_receive(struct uart_port *port)
{
	unsigned int stat = IO_PORT_INT_STATUS(port);
	struct tty_struct *tty = port->state->port.tty;
	unsigned char ch = 0;
	char flag = TTY_NORMAL;
	struct ubicom32_io_port *io_port = IO_PORT(port);

	if ((stat & (IO_PORTX_INT_SERDES_RXBF | IO_PORTX_INT_FIFO_RX_OF)) == 0) {
		return;
	}

	if (stat & IO_PORTX_INT_FIFO_RX_OF) {
		port->icount.overrun++;
	}
	...
	if (stat & IO_PORTX_INT_SERDES_RXBF) {
		int i;
		int nchars = io_port->fifo_level & 0xFFFF;

		port->icount.rx += nchars;

		for (i = 0; i < nchars; i++) {
			ch = ubi32_serdes_get_char(io_port);
...
{
	IO_PORT_INT_MASK(port) &= ~IO_PORTX_INT_SERDES_TXBE;
}

/*
 * ubi32_serdes_handle_transmit
 *	Returns 1 if busy, 0 otherwise
 */
static void ubi32_serdes_handle_transmit(struct uart_port *port)
{
	unsigned int stat = IO_PORT_INT_STATUS(port);
	struct circ_buf *xmit  = &port->state->xmit;

	/*
	 * Check to see if it was a TX buffer empty int
	 */
...
static void ubi32_serdes_start_tx(struct uart_port *port)
{
	...
	//ubi32_serdes_handle_transmit(port, IO_PORT_INT_STATUS(port));
}
...
static irqreturn_t ubi32_serdes_isr(int irq, void *dev_id)
{
	...
	ubi32_serdes_handle_receive(port);
	ubi32_serdes_handle_transmit(port);
...
static unsigned int ubi32_serdes_tx_empty(struct uart_port *port)
{
	...
	return (ret & IO_PORTX_INT_SERDES_TXBE) ? TIOCSER_TEMT : 0;
}
...
{
#ifdef IP8000
	if (request_irq(port->irq + 1, ubi32_serdes_isr, IRQF_DISABLED,
	     "UBI32_SERDES 2", port)) {
	...
	IO_PORT_INT_SET(port) = IO_PORTX_INT_FIFO_RX_RESET;
	IO_PORT_INT_CLR(port) = IO_PORTX_INT_SERDES_RXBF |
				IO_PORTX_INT_FIFO_RX_OF |
				IO_PORTX_INT_FIFO_RX_WM;
		   ...
		   struct ktermios *old)
{
	...
	port->read_status_mask = IO_PORTX_INT_SERDES_RXBF | IO_PORTX_INT_FIFO_RX_OF;

	port->ignore_status_mask = 0;
	if (termios->c_iflag & IGNPAR)
		port->ignore_status_mask |= IO_PORTX_INT_FIFO_RX_OF;

	/* ignore all characters if CREAD is not set */
	if ((termios->c_cflag & CREAD) == 0)
		port->ignore_status_mask |=
			IO_PORTX_INT_SERDES_RXBF | IO_PORTX_INT_FIFO_RX_OF;
...
MODULE_AUTHOR("Rainer Keller <rkeller@ubicom.com>, Patrick Tjin <pattjin@ubicom.com>");
MODULE_DESCRIPTION("Ubicom SERDES serial port driver");
MODULE_LICENSE("GPL");
