 ...
 *
 * <idmap>    := <dev-id>=<mtd-id>
 * <dev-id>   := 'nand'|'nor'|'onenand'|'spier'|<dev-num>
...
/**
 * Performs sanity check for supplied SPI-ER NAND flash partition.
 * Table of existing SPI-ER NAND flash devices is searched and partition device
 * is located. Alignment with the granularity of nand erasesize is verified.
 *
 * @param id of the parent device
 * @param part partition to validate
 * @return 0 if partition is valid, 1 otherwise
 */
static int part_validate_spier(struct mtdids *id, struct part_info *part)
{
#if defined(CONFIG_CMD_NAND_SPI_ER)
	/* info for SPI-ER NAND chips */
	struct mtd_info *mtd;
...
static int part_validate(struct mtdids *id, struct part_info *part)
{
	...
	else if (id->type == MTD_DEV_TYPE_NANDSPIER)
		return part_validate_spier(id, part);
	else
...
#endif
	} else if (type == MTD_DEV_TYPE_NANDSPIER) {
#if defined(CONFIG_CMD_NAND_SPI_ER)
		*size = nand_spi_er_mtd.size;
		return 0;
#else
		printf("support for SPI-ER NAND devices not present\n");
#endif
	} else
...
/**
 * Parse device id string <dev-id> := 'nand'|'nor'|'onenand'|'spier'<dev-num>,
	...
	*dev_type = 0;
	if (strncmp(p, "nand", 4) == 0) {
		...
		*dev_type = MTD_DEV_TYPE_ONENAND;
		p += 7;
	} else if (strncmp(p, "spier", 5) == 0) {
		*dev_type = MTD_DEV_TYPE_NANDSPIER;
		p += 5;
	} else {
	...
	if (!isdigit(*p)) {
		printf("incorrect device number in %s '%s'\n", id, p);
		return 1;
	}
...
#define PART_ADD_DESC_MAXLEN 64
		...
		if ((device_parse(tmpbuf, &unused, &dev) != 0) || (!dev))
			return 1;
