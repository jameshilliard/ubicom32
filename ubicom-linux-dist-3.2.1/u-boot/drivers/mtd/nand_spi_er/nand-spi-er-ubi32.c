...
unsigned int nand_spi_er_switch_port = 0;
unsigned int nand_spi_er_switch_pin = 0;
static int nand_spi_er_switch = 0;

#define GRAB_XFL()	if (nand_spi_er_switch) {							\
				struct ubicom32_io_port *xfl = (struct ubicom32_io_port *)FLASH_PORT;	\
				xfl->ctl0 |= IO_XFL_CTL0_MCB_LOCK;					\
				gpio_set_value(nand_spi_er_switch, 1);					\
			}
#define RELEASE_XFL()	if (nand_spi_er_switch) {							\
				struct ubicom32_io_port *xfl = (struct ubicom32_io_port *)FLASH_PORT;	\
				gpio_set_value(nand_spi_er_switch, 0);					\
				xfl->ctl0 &= ~IO_XFL_CTL0_MCB_LOCK;					\
			}
 ...
 */
static uint8_t nand_spi_er_get_feature(uint32_t reg)
{
...
/*
 * nand_spi_er_send_rd_addr
 *	perform FC_RD: CMD + address
 */
static void nand_spi_er_send_rd_addr(uint8_t command, uint32_t address)
{
	struct ubicom32_io_port *io = (struct ubicom32_io_port *)FLASH_PORT;
...
/*
 * nand_spi_er_send_cmd_addr
 *	perform FC_(xxx): CMD + address
 */
static void nand_spi_er_send_cmd_addr(uint8_t command, uint32_t address)
{
	struct ubicom32_io_port *io = (struct ubicom32_io_port *)FLASH_PORT;
...
/*
 * nand_spi_er_write_disable
 *	clear the write enable bit
 */
static void nand_spi_er_write_disable(void)
{
	struct ubicom32_io_port *io = (struct ubicom32_io_port *)FLASH_PORT;
...
/*
 * nand_spi_er_write_enable
 *	set the write enable bit
 */
static void nand_spi_er_write_enable(void)
{
	struct ubicom32_io_port *io = (struct ubicom32_io_port *)FLASH_PORT;
 ...
 */
static uint8_t nand_spi_er_busywait(void)
{
	int i;
	uint8_t data;
	 ...
	 */
	for (i = 0; i < 200; i++) {
		data = nand_spi_er_get_feature(0xC0);
...
static int nand_spi_er_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	MTDDEBUG(MTD_DEBUG_LEVEL3, "%s: erase addr:%x len:%x\n", chip->name, instr->addr, instr->len);
	...
	if (instr->addr & (chip->device->erase_size - 1)) {
		MTDDEBUG(MTD_DEBUG_LEVEL1, "%s: erase address is not aligned %x\n", chip->name, instr->addr);
		return -EINVAL;
	}

	if (instr->len & (chip->device->erase_size - 1)) {
		MTDDEBUG(MTD_DEBUG_LEVEL1, "%s: erase len is not aligned %x\n", chip->name, instr->len);
		return -EINVAL;
	}
	...
	while (instr->len) {
		...
		uint8_t stat;
		MTDDEBUG(MTD_DEBUG_LEVEL3, "%s: block erase row:%x block:%x addr:%x rem:%x\n", chip->name, row, block, instr->addr, instr->len);
		 ...
		 * Block erase
		 */
		nand_spi_er_send_cmd_addr(0xD8, row);

		/*
		 * Wait
		 */
		stat = nand_spi_er_busywait();
		if (stat & NAND_SPI_ER_STATUS_OIP) {
			instr->fail_addr = block << 17;
			instr->state = MTD_ERASE_FAILED;
			MTDDEBUG(MTD_DEBUG_LEVEL1, "%s: chip is busy or nonresponsive stat=%02x\n", chip->name, stat);
		...
		if (stat & NAND_SPI_ER_STATUS_E_FAIL) {
			MTDDEBUG(MTD_DEBUG_LEVEL1, "%s: E_FAIL signalled (%02x)\n", chip->name, stat);
			instr->fail_addr = block << 17;
			instr->state = MTD_ERASE_FAILED;
			goto done;
		}
				  ...
				  size_t *retlen, u_char *buf)
{
	...
	*retlen = 0;
	MTDDEBUG(MTD_DEBUG_LEVEL2, "%s: read block from %llx len %d into %p\n", chip->name, from, len, buf);
	...
	row = from >> 11;
	column = from & 0x7FF;
	MTDDEBUG(MTD_DEBUG_LEVEL3, "%s: row=%x %d column=%x %d last_row=%x %d\n", chip->name, row, row, column, column, chip->last_row, chip->last_row);
	...
	while (len) {
		...
		MTDDEBUG(MTD_DEBUG_LEVEL3, "%s: buf=%p toread=%x row=%x column=%x last_row=%x\n", chip->name, pbuf, toread, row, column, chip->last_row);
		...
		if (chip->last_row != row) {
			 ...
			 */
			nand_spi_er_send_cmd_addr(0x13, row);
			...
			stat >>= 4;
			if (stat == 1) {
				MTDDEBUG(MTD_DEBUG_LEVEL1, "%s: ECC recovered, row=%x\n", chip->name, row);
				retval = -EUCLEAN;
			}
			if (stat == 2) {
				MTDDEBUG(MTD_DEBUG_LEVEL0, "%s: failed ECC, row=%x\n", chip->name, row);
				chip->last_row = NAND_SPI_ER_LAST_ROW_INVALID;
				//mutex_unlock(&chip->lock);
				RELEASE_XFL();
				return -EBADMSG;
		...
		if (((uint32_t)pbuf & 0x03) == 0) {
			...
			tmp = toread & (~0x03);
			for (i = 0; i < tmp; i += 4) {
				nand_spi_er_send_rd_addr(0x03, column << 8);
				*pbuf++ = io->status1;	
				column += 4;
			}
		} else {
				   ...
				   size_t *retlen, const u_char *buf)
{
	...
	MTDDEBUG(MTD_DEBUG_LEVEL2, "%s: write block to %llx len %d from %p\n", chip->name, to, len, buf);
	...
	*retlen = 0;
	...
	while (len) {
		...
		MTDDEBUG(MTD_DEBUG_LEVEL3, "%s: write %p to row:%x col:%x len:%x rem:%x\n", chip->name, buf, row, col, towrite, len);
		 ...
		 * Program execute
		 */
		nand_spi_er_send_cmd_addr(0x10, row);
...
static int nand_spi_er_isbad(struct mtd_info *mtd, loff_t ofs)
{
	...
	if (ofs & (chip->device->erase_size - 1)) {
		MTDDEBUG(MTD_DEBUG_LEVEL1, "%s: address not aligned %llx\n", chip->name, ofs);
		return -EINVAL;
	}
...
static int nand_spi_er_markbad(struct mtd_info *mtd, loff_t ofs)
{
	...
	asm volatile (
		...
		:
		: [word1] "d" (0x0800dead | (NAND_SPI_ER_BAD_BLOCK_MARK_OFFSET << 16)),
		  [word2] "d" (0xbeef0000),
		  [port] "a" (FLASH_PORT)
		: "cc"
	);
...
static int nand_spi_er_read_bbt(struct nand_spi_er *chip)
{
	...
	for (j = 0; j < chip->device->blocks; j++) {
		 ...
		 */
		nand_spi_er_send_rd_addr(0x03, 0x080000);
		...
		nand_spi_er_send_rd_addr(0x03, 0x080000 | (NAND_SPI_ER_BAD_BLOCK_MARK_OFFSET << 8));
 ...
 */
int nand_spi_er_probe(struct mtd_info *mtd)
{
	uint32_t i;
	uint16_t id;
	int res;
	...
	/*
	 * The port could be 0 (IO_PORT_RA) but then the pin cannot be 0.
	 * The pin could be 0 but then the port cannot be 0.
	 * (PA0 is used by the flash controller and thus not usable as GPIO)
	 */
	if (nand_spi_er_switch_port || nand_spi_er_switch_pin) {
		nand_spi_er_switch = (nand_spi_er_switch_port * 32) + 
					  nand_spi_er_switch_pin;
		printf(DRIVER_NAME ": switch at gpio %d\n", nand_spi_er_switch);
		gpio_direction_output(nand_spi_er_switch, 0);
	}
	...
	id = io->status1 & 0xFFFF;

	printf(DRIVER_NAME ": found device id %08x\n", id);

	device = nand_spi_er_devices;
	for (i = 0; i < ARRAY_SIZE(nand_spi_er_devices); i++) {
		/*
		 * Work around a bug where, at higher frequencies, we get
		 * a swapped ID
		 */
		if (device->id == ((id >> 8) | (id << 8))) {
			break;
		}
		if (device->id == id) {
			break;
	 ...
	 */
	bbt_bytes = DIV_ROUND_UP(device->blocks, 8);
	chip = kzalloc(sizeof(struct nand_spi_er) + bbt_bytes, GFP_KERNEL);
	if (!chip) {
		RELEASE_XFL();
		return -ENOMEM;
	}
	sprintf(chip->name, "%s", device->name);
	mtd->priv = chip;
	 ...
	 */
	mtd->size = device->blocks * device->pages_per_block * device->page_size;
	mtd->erasesize = device->erase_size;

	/*
	 * 1 page, optionally we can support partial write (512)
	 */
	mtd->writesize = device->write_size;
	mtd->name = DRIVER_NAME;
	mtd->erase = nand_spi_er_erase;
	mtd->read = nand_spi_er_read;
	mtd->write = nand_spi_er_write;
	mtd->block_isbad = nand_spi_er_isbad;
	mtd->block_markbad = nand_spi_er_markbad;
	mtd->priv = chip;
	...
	printk(KERN_INFO "%s: added device %s size: %u KBytes %lu bad blocks %s\n", mtd->name, device->name, DIV_ROUND_UP(mtd->size, 1024), chip->nbb, read_only ? "[read only]" : "");
 ...
 * nand_spi_er_remove
 */
int nand_spi_er_remove(struct mtd_info *mtd)
{
	int status;

	//MTDDEBUG(MTD_DEBUG_LEVEL1, "%s: remove\n", mtd);

	status = del_mtd_device(mtd);
	if (status == 0) {
		kfree(mtd->priv);
		mtd->priv = NULL;
	}
