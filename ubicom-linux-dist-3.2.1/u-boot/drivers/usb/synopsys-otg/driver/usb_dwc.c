 ...
 */
static void dwc_otg_overide(u32 fbase, int is_host)
{
 ...
 */
static void dwc_otg_cfg_mode(u32 fbase, int inst)
{
	/*
 ...
 */
static void dwc_bringup_port(int inst)
{
	...
	printf("Parameter Override Signals:  0x%08X\n", reg );
	...
	*/
	DWC_REG_SET(fbase + IO_CTL1, DWC_PHYENABLE_BIT);
       /*
	* Enable the interrupt for function F  in int_mask2
	*/
	ubicom32_enable_interrupt( RF_USB20_INT);
       /*
	* enable the interrupt for function G  in int_mask2
	*/
	ubicom32_enable_interrupt( RG_USB20_INT);
	/*
	...
	printf(">>>>>>>>>> IO_INT_MASK= 0x%08X\n", reg );

	DWC_REG_READ(fbase + IO_STATUS1, reg);
	printf(">>>>>>>>>> IO_STATUS1= 0x%08X\n", reg );
#endif
 ...
 */
static unsigned int dwc_get_irq(int inst)
{
 ...
 */
static void dwc_init_and_register_device(int inst)
{
        printf("%s: %d\n", __func__, inst);
	dwc_bringup_port(inst);
	printf("USB 2.0 port %d is brought up\n", inst);

	dwc_devices[inst].irq = dwc_get_irq(inst);
	printf("USB 2.0 port %d irq = %u\n", inst, dwc_devices[inst].irq);
}

/*
 * Usb 2.0 subsystem init
 */
int ubi32_dwc_init(void)
{
        printf("%s:\n", __func__);
 ...
 */
void ubi32_dwc_dma_ready(void *fbasep)
{
	...
	printf("dma_ready wcmdq(%x, %p), stall(%x, %p)\n", fbase + IO_STATUS1,
	 ...
	 */
        ...
        if (DWC_REG_TST(fbase + IO_STATUS1, DWC_STATUS1_WCMDQ_EMPTY_BIT)) {
                return;
	}
