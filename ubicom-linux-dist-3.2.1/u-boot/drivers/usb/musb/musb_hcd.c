...
static void write_toggle(struct usb_device *dev, u8 ep, u8 dir_out)
{
	...
	if (dir_out) {
		if (!toggle)
			musb_writew(MUSB_TXCSR_MODE | MUSB_TXCSR_CLRDATATOG, &musbr->txcsr);
		else {
		...
		musb_writew(csr, &musbr->txcsr);
		if (!toggle)
			musb_writew(MUSB_RXCSR_FLUSHFIFO | MUSB_RXCSR_CLRDATATOG, &musbr->rxcsr);
		else {
					...
					void *buffer, int len)
{
	...
	u32 nextlen = 0;
	u8  devspeed;
	u8  musb_bulk_ep = MUSB_BULKIN_EP;

	if (dir_out)
		musb_bulk_ep = MUSB_BULK_EP;

	/* select bulk endpoint */
	musb_writeb(musb_bulk_ep, &musbr->index);

	/* write the address of the device */
	if (dir_out)
		musb_writeb(devnum, &musbr->tar[musb_bulk_ep].txfuncaddr);
	else
		musb_writeb(devnum, &musbr->tar[musb_bulk_ep].rxfuncaddr);
	...
	if ((musb_ishighspeed()) && (dev->parent != NULL) &&
		...
		if (dir_out) {
			musb_writeb(0, &musbr->tar[musb_bulk_ep].txhubaddr);
			musb_writeb(0, &musbr->tar[musb_bulk_ep].txhubport);
		} else {
			musb_writeb(0, &musbr->tar[musb_bulk_ep].rxhubaddr);
			musb_writeb(0, &musbr->tar[musb_bulk_ep].rxhubport);
		}
	...
	if (dir_out) { /* bulk-out transfer */
		...
		while (txlen < len) {
			...
			/* Set the ReqPkt bit */
			musb_writew(MUSB_RXCSR_H_REQPKT, &musbr->rxcsr);
...
int usb_lowlevel_init(void)
{
	...
	while (timeout--) {
		if (musb_readb(&musbr->intrusb) & MUSB_INTR_CONNECT) {
			break;
		}
		if (musb_readb(&musbr->devctl) & MUSB_DEVCTL_SESSION) {
			break;
		}
	}
	...
	if (timeout < 1) {
		devctl = 0;
		while (1) {
			power = musb_readb(&musbr->devctl);
			if (devctl != power)
				printf("3 devctl: %02x -> %02x\n", devctl, power);
			devctl = power;
		}
		return -1;
	}
	...
	return (musb_cfg.musb_speed == MUSB_TYPE_SPEED_LOW);
}
