...
s32 /*__init*/ synopGMAC_init_network_interface(void)
{
	...
	for (i = 0; i < 1; i++) {
		...
		if (eth_node->dn.recvirq == RH_GMAC_INT) {
			gmacdev->synopGMACMappedAddr = IO_PORT_RH;
		} else {
			gmacdev->synopGMACMappedAddr = IO_PORT_RI;
		}
		TR0("Initializing synopsys GMAC interfaces %s: (port = 0x%x, irq = %d)\n",
			eth_node->dn.name, gmacdev->synopGMACMappedAddr, eth_node->dn.recvirq) ;
...
int eth_send(volatile void *packet, int len)
{
	...
	synopGMAC_linux_cable_unplug_function((u32)gmacdev);
...
void eth_halt(void)
{
	...
	if (synopGMACDev[0] && synopGMACDev[0]->ethdn->command != 0) {
