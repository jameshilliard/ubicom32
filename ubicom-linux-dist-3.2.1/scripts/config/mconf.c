...
"Overview\n"
"--------\n"
"Some Ubicom distro features may be built directly into the image.\n"
...
"between different Ubicom distro configurations.\n"
"\n"
...
load_config_help[] = N_(
	"\n"
	"For various reasons, one may wish to keep several different Ubicom Distro\n"
	"configurations available on a single machine.\n"
	"\n"
	"If you have saved a previous configuration in a file other than\n"
	"Ubicom Distro's default, entering the name of the file here will allow you\n"
...
save_config_help[] = N_(
	"\n"
	"For various reasons, one may wish to keep different Ubicom Distro\n"
	"configurations available on a single machine.\n"
	"\n"
...
int main(int ac, char **av)
{
	...
	sym = sym_lookup("UBICOMDISTROVERSION", 0);
	sym_calc_value(sym);
	sprintf(menu_backtitle, _("Ubicom Distro %s Configuration"),
		sym_get_string_value(sym));
	...
	do {
		cprint_init();
		cprint("--yesno");
		cprint(_("Do you wish to save your new Ubicom Distro configuration?"));
		cprint("5");
		cprint("60");
		stat = exec_conf();
	} while (stat < 0);
	...
	if (stat == 0) {
		if (conf_write(NULL)) {
			fprintf(stderr, _("\n\n"
				"Error during writing of the Ubicom Distro configuration.\n"
				"Your configuration changes were NOT saved."
				"\n\n"));
			return 1;
		}
		printf(_("\n\n"
			"*** End of Ubicom Distro configuration.\n"
			"*** Execute 'make' to build the Ubicom Distro or try 'make help'."
			"\n\n"));
	} else {
		fprintf(stderr, _("\n\n"
			"Your configuration changes were NOT saved."
