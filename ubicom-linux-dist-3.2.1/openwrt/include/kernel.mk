  ...
  ifneq ($(if $(filter-out %=y %=n %=m,$(KCONFIG)),$(filter m,$(foreach c,$(filter-out %=y %=n %=m,$(KCONFIG)),$($(c)))),.),)
    ifneq ($(strip $(FILES)),)
      define Package/kmod-$(1)/install
		  $(eval LINUX_REAL_VERSION=`cat $(LINUX_DIR)/include/config/kernel.release`)
		  mkdir -p $$(1)/lib/modules/$(LINUX_REAL_VERSION)
		  $(CP) -L $$(FILES) $$(1)/lib/modules/$(LINUX_REAL_VERSION)/
