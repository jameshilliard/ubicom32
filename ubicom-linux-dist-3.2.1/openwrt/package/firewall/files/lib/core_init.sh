...
fw_load_defaults() {
	...
	fw add i f urlfilter_rule
	fw add i f FORWARD urlfilter_rule
...
fw_load_porttrigger() {
	fw_config_get_section "$1" porttrigger { \
		string trigger_proto "" \
		string trigger_port 0 \
		string forward_proto "" \
		string forward_port 0 \
	} || return

	local lanifname=$(uci -P/var/state get network.lan.ifname)

	local wan_ifs=$(fw_get_wan_ifs)
	for wan_if in $wan_ifs
	do
		local wanip=$(uci -P/var/state get network.${wan_if}.ipaddr)
		local wandev=$(uci -P/var/state get network.${wan_if}.ifname)
		if [ -n "$wanip" ]; then
			fw add i f forwarding_rule PORTTRIGGER { \
				-i $lanifname -o $wandev -p $porttrigger_trigger_proto \
				--mode forward_out --trigger-proto $porttrigger_trigger_proto \
				--forward-proto $porttrigger_forward_proto --trigger-ports \
				$porttrigger_trigger_port --forward-ports $porttrigger_forward_port \
			}
			fw add i f forwarding_rule PORTTRIGGER { -i $wandev -o $lanifname \
				-p $porttrigger_forward_proto  --mode forward_in }
			fw add i n prerouting_rule PORTTRIGGER { -i $wandev -d $wanip --mode dnat }
		fi
	done
}

fw_load_vservice() {
	fw_config_get_section "$1" vservice { \
		string proto "" \
		string addr "" \
		string port 0 \
	} || return
	...
	local netmask=$(uci -P/var/state get network.lan.netmask)
	local lannet=$lanip/$netmask
	...
	do
		local wanip=$(uci -P/var/state get network.${wan_if}.ipaddr)
		if [ -n "$wanip" ]; then
			proto_list="$vservice_proto"
			[ "$proto_list" == "ALL" ] && proto_list="tcp udp"
			for proto in $proto_list
			do
				fw add i f forwarding_rule ACCEPT { \
					-o $lanifname -p $proto -d $vservice_addr \
					--dport $vservice_port -m state --state NEW }

				fw add i n prerouting_rule DNAT { \
					-p $proto -d $wanip --dport $vservice_port \
					--to-destination $vservice_addr:$vservice_port }

				fw add i n postrouting_rule SNAT { -p $proto -s $lannet \
					-d $vservice_addr --dport $vservice_port --to-source $wanip }
			done
		fi
	done
}
...
fw_load_urlfilter() {
	fw_config_get_section "$1" urlfilter { string policy "DISABLE" } || return

	[ "$urlfilter_policy" == "DISABLE" ] && return

	add_url() {
		if [ "$urlfilter_policy" == "DROP" ]; then
			fw add i f urlfilter_rule zone_lan_REJECT { -m urlfilter --url $1 }
		fi

		if [ "$urlfilter_policy" == "ACCEPT" ]; then
			fw add i f urlfilter_rule ACCEPT { -m urlfilter --url $1 }
		fi
	}

	config_list_foreach $1 url add_url

	[ $urlfilter_policy == "ACCEPT" ] && fw add i f urlfilter_rule zone_lan_REJECT { -m urlfilter --url ALL }
}
...
fw_load_dmz() {
	...
	[ -n "$defaults_dmz_host" ] && {
		...
		do
			...
			[ -n "$wanip" ] && {
				fw add i n zone_wan_prerouting DNAT { -d $wanip --to $defaults_dmz_host }
				fw add i f forwarding_rule ACCEPT { -d $defaults_dmz_host -i $wandev -o $lanifname -m state --state NEW }
			}
		done
	}
}
...
fw_load_wan_ping() {
	...
	[ $defaults_wan_ping == 1 ] && {
		...
		do
			...
			[ -n "$wanip" ] && {
				fw add i f input_rule ACCEPT { -p icmp --icmp-type echo-request -i $wandev -d $wanip }
			}
		done
	}
}
...
fw_load_rmt_mng() {
	fw_config_get_section "$1" defaults { \
		string rmt_mng_port 0 \
	} || return

	[ "$defaults_rmt_mng_port" != "0" ] && {
		local wan_ifs=$(fw_get_wan_ifs)
		for wan_if in $wan_ifs
		do
			local wanip=$(uci -P/var/state get network.${wan_if}.ipaddr)
			...
			[ -n "$wanip" ] && {
				fw add i n prerouting_rule DNAT { -p TCP -d $wanip --dport $defaults_rmt_mng_port --to $wanip:80 }
				fw add i f input_rule ACCEPT { -p TCP -d $wanip --dport 80 -m conntrack --ctorigdstport $defaults_rmt_mng_port }
			}
		done
	}
}
...
fw_load_nat_type() {
	fw_config_get_section "$1" defaults { \
		string tcp_nat_type 0 \
		string udp_nat_type 0 \
	} || return

	add_nat_type() {
		fw add i f forwarding_rule NATTYPE { -i $lanifname -o $wandev -p $1 --mode forward_out --type $2 }
		fw add i f forwarding_rule NATTYPE { -i $wandev -o $lanifname -p $1 --mode forward_in --type $2 }
		fw add i n prerouting_rule NATTYPE { -i $wandev -p $1 --mode dnat --type $2 }
	}
	...
	do
		...
		[ -n "$defaults_tcp_nat_type" -a "$defaults_tcp_nat_type" != "0" ] && add_nat_type "TCP" $defaults_tcp_nat_type
		[ -n "$defaults_udp_nat_type" -a "$defaults_udp_nat_type" != "0" ] && add_nat_type "UDP" $defaults_udp_nat_type
	done
}
