	...
	case "$iftype" in
		bridge)
			...
			[ -x /usr/sbin/brctl ] && {
				...
				# First bring down interface to be able to set MAC addr.
				ifconfig "$iface" down
...
setup_interface_static() {
	...
	[ -z "$dns" ] || {
		...
		# so that it gives the WAN side static DNS server addresses to the LAN DHCP clients when DNS relay is disabled.
		...
		uci set dhcp.lan.dhcp_option=6$dns_servers
		uci commit dhcp
...
setup_interface() {
	...
	case "$proto" in
		...
		dhcp)
			...
			[ $config = "lan" -a "${iface}" = "br-${config}" ] && {
				udhcpc -f -nq $iface || {
					AUTO_IP=`echo $$ | awk '{print ($1-1234)%256 "." $1%256}'`
			#      		echo no IP and no dhcp server found -- use $AUTO_IP
				}
			}
			...
			[ -n "$AUTO_IP" ] && {
				sleep 2 # dhcp kills IPs, so wait till it sleeps
				ifconfig $iface 169.254.$AUTO_IP/16 up
			}
		;;
		none)
