// ip4kcopro.cxx - ip4k coprocessor sid encapsulation.  -*- C++ -*-
...
ip4kcopro::ip4kcopro (const string& conf_file_name) :
...
sid::bus::status 
ip4kcopro::read_data_memory (sid::host_int_4 addr,
			   unsigned int length,
			   sid::host_int_1 *buf)
{
...
sid::bus::status 
ip4kcopro::write_data_memory (sid::host_int_4 addr,
		  	    unsigned int length,
			    sid::host_int_1 *buf)
{
  for (unsigned int i = 0; i < length; ++i)
...
sid::bus::status 
ip4kcopro::read_insn_memory (sid::host_int_4 addr,
			   unsigned int length,
			   sid::host_int_1 *buf)
{
  for (unsigned int i = 0; i < length; ++i)
...
sid::bus::status 
ip4kcopro::write_insn_memory (sid::host_int_4 addr,
		  	    unsigned int length,
			    sid::host_int_1 *buf)
{
  for (unsigned int i = 0; i < length; ++i)
...
ip4kcopro::read_register (ip4kcopro::regno_t regno,
...
sid::component::status 
ip4kcopro::write_register (ip4kcopro::regno_t regno,
			   sid::host_int_8 value)
{
  this->check_load ();
  string regname = string("gdb-register-") + sidutil::make_numeric_attribute (regno);
...
sid::component::status
ip4kcopro::interrupt (unsigned irq_num)
{
  return component::not_found;
}

void
ip4kcopro::checkpoint_all_state (std::ostream& file)
{
...
sid::component::status
ip4kcopro::restore_all_state (std::istream& file)
{
