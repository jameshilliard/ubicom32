/* ip3kcpu sid encapsulation library test case 2
# mach(): ip3k

*/

// This test case is designed to load a program, manipulate it so it will
// print out "pass", then save the state to a file.  The original ip3kcpu
// instance is destroyed without running the program to completion.  A
// new ip3kcpu instance is created and restored to the state saved earlier.
...
int main()
{
  ...
  ip3kcpu *newcpu = new ip3kcpu("ip3kcpu.out.conf");

  newcpu->load ("ip3kcpu.out");

  save_state.seekp(0);
  newcpu->restore_all_state (save_state);

  d4 = 0;
  newcpu->read_register (ip3kcpu::d4, d4);
  CHECK(d4 == 5);

  newcpu->read_register (ip3kcpu::pc, pc);
  CHECK(pc == 12);

  newcpu->read_register (ip3kcpu::d3, d3);
