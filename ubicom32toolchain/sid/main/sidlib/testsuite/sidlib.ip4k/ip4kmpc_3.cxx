/* ip4k_main_plus_copro sid encapsulation library test case 3
# mach(): ip4k
  
*/
  
// This test case is designed to load a program, manipulate it and then
// save the state via the map interface.  Without closing the instance,
// the program is further manipulated, then restored via the map.  It
// is then run to completion so it will print "pass".
...
int main()
{
  ...
  half = 0x0005;     
  mycpu->write_register_main (ip4kcpu::d4, half);
  ...
  big_int_8 wrong_msg;
  wrong_msg.write_byte(0, 'o');
  wrong_msg.write_byte(1, 'k');
  wrong_msg.write_byte(2, ' ');
  wrong_msg.write_byte(3, 'f');
  wrong_msg.write_byte(4, 'i');
  wrong_msg.write_byte(5, 'n');
  wrong_msg.write_byte(6, 'e');
  wrong_msg.write_byte(7, '\n');
  mycpu->write_data_memory_main (d12, wrong_msg);

  mycpu->read_data_memory_main (d12, top_byte);
  CHECK (top_byte.integer_value() == 'o');
  ...
  creg = 1;
  mycpu->write_register_copro (ip4kcopro::r1, creg);
  creg = 2;
  mycpu->write_register_copro (ip4kcopro::r2, creg);
  creg = 0x207e;
  mycpu->write_register_copro (ip4kcopro::r4, creg);
