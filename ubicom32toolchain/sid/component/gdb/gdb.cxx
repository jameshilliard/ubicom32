...
gdb::process_get_gen ()
{
  ...
  if (gdbserv_input_string_match (gdbserv, "C") >= 0)
    {
      /* Context query */
      int reg_nr = 66;
      string reg_value;
	...
	num_regs = 200;
      ...
      if(num_regs < 200)
	{
	  string reg_name = string("ubicom_thread_select");
	  reg_value = cpu->attribute_value (reg_name);
	  ...
	  if (reg_value.size() > 0)
		...
		target_value.write_byte (i, reg_value[i]);
	      ...
	      int context = (target_value + 1);

	      char buffer[10];
	      sprintf(buffer, "QC%d", context);
	      gdbserv_output_string (gdbserv, buffer);
...
gdb::process_set_gen ()
{
  ...
  /* peek the next letter and see if it is - */
  byte = gdbserv_input_peek (gdbserv);
  ...
  if(byte == '-')
    {
      ...
      /* Suck out the - */
      byte = gdbserv_input_char (gdbserv);
    }
  /* read the rest as hex number */
  unsigned long myThreadNo;
  byte = gdbserv_input_hex_ulong (gdbserv, &myThreadNo);
  ...
  if(num_regs < 200)
    {
      if(ignore == 0)
	cpu->set_attribute_value(string("ubicom_thread_select"), reg_image);
    }
	     
  //printf("threadNo = %d tVal = %d ignore = %d\n", myThreadNo, tVal, ignore);
  ...
  if(num_regs < 200)
    {
      ...
      /* go get the thread #  */
      string reg_name = string("ubicom_thread_select");
      string reg_value = cpu->attribute_value (reg_name);
      ...
      if (reg_value.size() > 0)
	{
      ...
      int context = target_value;
      char buffer[30];
      sprintf(buffer, "thread:%d;", (context+1));
      gdbserv_output_string(gdbserv, buffer);
    }
  else
    {
