...
#if defined(TARGET_ubicom32)
struct moveai_info {
    arelent * lower_bits_rela; 
    arelent * moveai_rela; 
    unsigned long upper_bits;	
    unsigned long lower_bits;	
} moveai_relocations[8];

int
ubicom32_flat_reloc(char *sectionp, uint32_t **flat_relocs, arelent **p, arelent *q, int *flat_reloc_count, int relcount, long section_vma, bfd *abs_bfd)
{
    /* A pointer to what's being relocated, used often
       below.  */
    unsigned char *r_mem = sectionp + q->address;
    unsigned char *lo_mem;
    int rc = 0;
    const char		*sym_name, *section_name;
    long sym_addr, sym_vma;
    asection *sym_section;
    flagword flags;
    unsigned long insn;
    unsigned long current_reg, address_bits, lo7_register, hi24_register;
    unsigned long top, bottom, ret;
    bfd_vma relocation;
    uint32_t *f_relocs = *flat_relocs;
    int num_relocs = *flat_reloc_count;
    ...
    insn = (r_mem[0] << 24) | (r_mem[1] << 16) |(r_mem[2] << 8) | r_mem[3];

    if ((flags & (BSF_SECTION_SYM|BSF_LOCAL)) == 0x101) {
	/*
	 * We have to find the relocation from the code.
	 */
	switch(q->howto->type) {
	case  R_UBICOM32_HI24:
	    {
		/*
		 * We have to find a matching LO7
		 */
		arelent *lo_q;
		int i;
		
		/*
		 * Extract the register and the 24 bits from the instruction.
		 */
		current_reg = (insn >> 21) & 0x7;
		
		/*
		 * Extract the instruction from the contents.
		 */
		address_bits = (insn & 0x1fffff) | ((insn >> 3) & 0xe00000);

		moveai_relocations[current_reg].moveai_rela = q;
		moveai_relocations[current_reg].lower_bits_rela = NULL;
		moveai_relocations[current_reg].upper_bits = (address_bits << 7);
		moveai_relocations[current_reg].lower_bits = 0;

		for(i = 1; i < relcount; i++) {
		    lo_q = p[i];

		    if (lo_q->howto->type == R_UBICOM32_HI24) {
			/*
			 * Hit another HI24, check that this HI24 supersedes the one we are currently searching for.
			 */
			lo_mem = sectionp + lo_q->address;
			insn = (lo_mem[0] << 24) | (lo_mem[1] << 16) | (lo_mem[2] << 8) | lo_mem[3];
			hi24_register = (insn >> 21) & 0x7;

			/*
			 * If we set another HI24 with the same register, we cannot continue.
			 */
			if (current_reg == hi24_register) {
			     break;
			}
			 ...
			 */
			continue;
		    } else if (lo_q->howto->type == R_UBICOM32_32) {
			/*
			 * No match. Keep looking.
			 */
			continue;
		    } else if (lo_q->sym_ptr_ptr != q->sym_ptr_ptr) {
			/*
			 * Keep looking.
			 */
			continue;
		    ...
		    /*
		     * Extract the insn and then pull out the register information and see if we have a match.
		     */
		    lo_mem = sectionp + lo_q->address;
		    insn = (lo_mem[0] << 24) | (lo_mem[1] << 16) |(lo_mem[2] << 8) | lo_mem[3];
		    
		    /*
		     * Now deal with the current instruction. Extract bits 0-4 and 8-9 of the instruction.
		     */
		    switch(lo_q->howto->type) {
		    case  R_UBICOM32_LO7_D:
		    case  R_UBICOM32_LO7_2_D:
		    case  R_UBICOM32_LO7_4_D:
			lo7_register = (insn >> 21) & 0x7;
			bottom = (insn >> 16) & 0x1f;
			top = (insn >> 24) & 0x3;
			ret = (top <<5) | bottom;
			if (lo_q->howto->type == R_UBICOM32_LO7_2_D) {
			    ret <<= 1;
			} else if (lo_q->howto->type == R_UBICOM32_LO7_4_D) {
			    ret <<= 2;
			}
			break;
			...
			lo7_register = (insn >> 5) & 0x7;
			bottom = (insn) & 0x1f;
			top = (insn >> 8) & 0x3;
			ret = (top <<5) | bottom;
			if (lo_q->howto->type == R_UBICOM32_LO7_2_S) {
			    ret <<= 1;
			} else if (lo_q->howto->type == R_UBICOM32_LO7_4_S) {
			    ret <<= 2;
			}
			break;
			...
			lo7_register = (insn >> 5) & 0x7;
			/*
			 * The address data is in bits 0-4 8-10 16-20 24-26
			 */
			ret = insn & 0x1f;
			insn >>= 8;
			ret |= (insn & 0x7) << 5;
		     ...
		     * We have a hit.
		     */
		    moveai_relocations[current_reg].lower_bits_rela = lo_q;
		    moveai_relocations[current_reg].lower_bits = ret;
		    break;
		}
		    
		relocation = moveai_relocations[current_reg].lower_bits | moveai_relocations[current_reg].upper_bits;
		break;
	    }
	case  R_UBICOM32_LO7_D:
	case  R_UBICOM32_LO7_2_D:
	case  R_UBICOM32_LO7_4_D:
		...
		top = (insn >> 24) & 0x3;
		ret = (top <<5) | bottom;
		if (q->howto->type == R_UBICOM32_LO7_2_D) {
		    ret <<= 1;
		} else if (q->howto->type == R_UBICOM32_LO7_4_D) {
		    ret <<= 2;
		}

		relocation = ret;

		if (moveai_relocations[lo7_register].moveai_rela) {
		    /*
		     * preexisting relocation. Or it in.
		     */
		    relocation |= moveai_relocations[lo7_register].upper_bits;
		}
		break;
	    }
	case  R_UBICOM32_LO7_S:
	...
	case  R_UBICOM32_LO7_4_S:
		...
		top = (insn >> 8) & 0x3;
		ret = (top <<5) | bottom;
		if (q->howto->type == R_UBICOM32_LO7_2_S) {
		    ret <<= 1;
		} else if (q->howto->type == R_UBICOM32_LO7_4_S) {
		    ret <<= 2;
		}
		relocation = ret;
	...
	if (q->howto->type == R_UBICOM32_HI24) {
	    /*
	     * Extract the register and the 24 bits from the instruction.
	     */
	    current_reg = (insn >> 21) & 0x7;
	...
	/*
	 * All are PC relative relocations already relosved by the linker.
	 */
	break;
    case  R_UBICOM32_LO7_D:
    case  R_UBICOM32_LO7_2_D:
    ...
    case  R_UBICOM32_LO16_CALLI:
	/*
	 * Allocate 2 slots. First slot contains r_type << 27| r_offset.
	 * Second slot contains the relocation.
	 */
	f_relocs = realloc(f_relocs,
			   (num_relocs + 2) * sizeof(uint32_t));
	f_relocs[num_relocs++] = (q->howto->type << 27) | (section_vma + q->address);
	f_relocs[num_relocs++] = relocation;
	break;

    case R_UBICOM32_32:
	  /*
	   * We are going to read the original contents and deal with it.
	   */
	f_relocs = realloc(f_relocs,
			   (num_relocs + 1) * sizeof(uint32_t));
	f_relocs[num_relocs++] = (q->howto->type << 27) | (section_vma + q->address);
	break;
    }

    *flat_relocs = f_relocs;
    *flat_reloc_count = num_relocs;
	...
	if (relcount <= 0) {
		...
		for (p = relpp; (relcount && (*p != NULL)); p++, relcount--) {
			...
			if (use_resolved) {
				...
				case  R_UBICOM32_LO16_CALLI:
				case R_UBICOM32_32:
				    (void ) ubicom32_flat_reloc(sectionp, &flat_relocs, p, q, &flat_reloc_count, relcount, section_vma, abs_bfd);
				    relocation_needed = 0;
				    pflags = ((*p)->howto->type) << 27;
				    break;
#elif defined(TARGET_arm)
				case R_ARM_ABS32:
					relocation_needed = 1;
...
#define OPCODE_SIZE 2           /* Add 2 bytes, counting the opcode size*/
			...
			if (relocation_needed) {
...
#if defined(TARGET_ubicom32)
				/*
				 * This is an 31 bit absolute relocation. We need the symbol address. What we are going to do is
				 * to take the 32 bit address and from it extract bits 26-0. We use the 4 MSB bits to differentiate between
				...
				/*
				 * We are going to be lazy and store the symbol address in the reloc table.
				 * This is going to appear as persistant.
				 */
				flat_relocs[flat_reloc_count++] = ((sym_addr + sym_section->vma) & 0x7fffffff);
#endif
				flat_relocs = realloc(flat_relocs,
					(flat_reloc_count + 1) * sizeof(uint32_t));
#ifndef TARGET_e1
