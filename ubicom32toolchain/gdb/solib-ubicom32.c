  ...
  while (lm_addr)
      ...
      if (got_addr != mgot)
	{
	  ...
	  if (errcode != 0)
	    ...
	    {
	      char buffer[SO_NAME_MAX_PATH_SIZE + 2];
	      char *ptr = strrchr(name_buf, '/');
	      if (ptr)
		ptr++;
	      else
		ptr = name_buf;
	      ...
	      strncpy (sop->so_name, buffer, SO_NAME_MAX_PATH_SIZE - 1);
	      sop->so_name[SO_NAME_MAX_PATH_SIZE - 1] = '\0';
	      strncpy (sop->so_original_name, name_buf, SO_NAME_MAX_PATH_SIZE - 1);
	      xfree (name_buf);
