...
// DJH - the comment above and the idea are quite nice in that optimizing for
// size probably does want to make things as small as possible, but this
// conditional stops us from enabling basic-block reordering even if we really
// want it.  For the Ubicom32 ISA BB reordering is a huge win and so we'll
// happily take the < 1% loss in size for the > 2% win in performance.
